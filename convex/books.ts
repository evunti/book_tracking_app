...
export const list = query({
  args: {
    sortBy: v.union(v.literal("title"), v.literal("author"), v.literal("rating"), v.literal("finished")),
  ...
  handler: async (ctx, args) => {
    ...
    const [allRatings, userRatings] = await Promise.all([
      ...
      userId ? ctx.db.query("ratings").withIndex("by_user", q => q.eq("userId", userId)).collect() : [],
    ]);
    ...
    for (const book of books) {
      ...
      if (bookRatings.length > 0) {
        const avg = bookRatings.reduce((acc, curr) => acc + curr.rating, 0) / bookRatings.length;
